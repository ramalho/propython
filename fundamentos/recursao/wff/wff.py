...
C_cube = 'CAKENZ' # changed R for Z to allow case-insensitive evaluation
s_cube = 'pqrsio'
...
def wff_gen(symbols):
    ...
    ens = [s for s in symbols if s == 'N']
    # discard useless symbols
    avail = [s for s in symbols if s in 'CAKEpqrs']
    # put Caps in front
...
def turn(dice, reply, num_dice, errors):
...
def demo_gerar_wffs():
    ...
    for i in range(3,11):
        for j in range(1, i):
            dados = wff_dice(i)
            wff = wff_gen(dados)
            res.append({'num_dados':i, 'dados':dados, 'wff':wff})
    return res  
...
if __name__ == '__main__':
    ...
    pprint(demo_gerar_wffs())
    
"""
Erro: os dados abaixo permitiriam sequencia mais longa,
mas programa aceitou resposta curta
     . A E N p p s q >>> napp
 8 | OK: NApp
"""
